...
// confirmer) · appui long sur un article = note et allée · appui long sur une
// liste = la glisser au rang voulu.

import { store, put, patch, remove } from "./store.js";
import { T } from "./textes.js";
import { icon } from "./icons.js";
import { esc, toast, openSheet, closeSheet, naviguer } from "./ui.js";
...
export function renderEpicerie(screen, dock) {
  els = { screen, dock };
  if (gesture?.active || isDragging()) return;        // pas de reconstruction sous le doigt
  ...
  // Un autre onglet vide le dock en passant : on le refait s'il est vide.
  if (dock.dataset.list !== ep.listId || !dock.firstElementChild) {
    dock.innerHTML = addBoxHtml();
    dock.dataset.list = ep.listId;
  }
  refreshList();
}
/** Onglet Courses touché alors qu'on y est déjà : retour à l'écran des listes. */
export function showLists() {
  if (!ep.listId) { if (els.screen) els.screen.scrollTop = 0; return; }
  naviguer("arriere", () => { openList(null); rerender(); if (els.screen) els.screen.scrollTop = 0; });
}
  ...
  const rows = all.map((l) => {
    ...
    return `
      <li class="list-row" data-id="${l.id}">
        ...
        <button class="icon-btn" data-ep="edit-list" data-no-drag data-id="${l.id}" aria-label="${T.modifierListe}">${icon("points")}</button>
      </li>`;
  }).join("");
  return `
    <div id="ep" data-list="">
      <header class="screen-head">
        <h1>${T.onglets.epicerie}</h1>
        <button class="btn btn-primary btn-icon round" data-ep="new-list" aria-label="${T.nouvelleListe}">${icon("plus")}</button>
      </header>
      ${all.length ? `<ul class="lists" id="ep-lists" data-reorder>${rows}</ul>` : `
        <div class="empty">
  ...
  return `
    <div id="ep" data-list="${ep.listId}">
      <header class="list-head">
        ...
        <h1 id="ep-title"></h1>
        <button class="icon-btn" data-ep="edit-list" data-id="${ep.listId}" aria-label="${T.modifierListe}">${icon("points")}</button>
      </header>
  ...
  return `
    <div class="ajout" id="ajout">
      ...
      <form class="ligne" id="ep-form">
        ...
        <button type="button" class="btn btn-primary btn-icon" data-ep="checked-menu" aria-label="${T.optionsCoches}">${icon("coches")}</button>
      </form>
    </div>`;
}
...
const aisleHeadHtml = (aisle) => `<h2 class="aisle-head">${esc(aisle?.name || T.autre)}</h2>`;
...
function refreshAddBox() {
  ...
  // synonymes). Pour ajouter le texte tel quel : « terminé » (ou Entrée).
...
function addItem(rawName) {
  ...
  if (!hi) {
    ...
    const sid = idStable(store.household.id, norm);
    const pris = store.data.household_items.some((h) => h.id === sid);   // article renommé depuis
...
function openAisleSheet(id) {
  ...
  openSheet(`
    <h2>${esc(i.name)}</h2>
    <div class="field">
      <label class="label" for="item-note">${T.note}</label>
      <input type="text" id="item-note" value="${esc(i.note || "")}" placeholder="${T.noteExemple}" autocomplete="off" maxlength="80" enterkeyhint="done">
    </div>
...
// La note s'enregistre dès qu'on quitte le champ (« terminé », ou fermer la feuille).
function saveNote(value) {
  const i = store.data.list_items.find((x) => x.id === sheetItemId);
  const note = value.replace(/\s+/g, " ").trim() || null;
  if (!i || (i.note || null) === note) return;
  patch("list_items", i.id, { note });
  refreshList();
}
...
// ---------------------------------------------------------- gestes : listes ---
// Bouton du bandeau d'ajout. (Le « … » de l'en-tête ouvre directement « Modifier la liste ».)
function openCheckedMenu() {
  openSheet(`
...
function saveListForm() {
  ...
  if (d.id) {
    ...
    ep.draft = null;
    closeSheet();
    naviguer("avant", () => { openList(l.id); rerender(); });   // on entre dans la nouvelle liste
    return;
  }
  ep.draft = null;
  closeSheet();
...
function deleteList() {
  ...
  // Ses articles aussi (le texte de confirmation le promet) : sinon ils restent
  // dans la base et se rechargent à chaque démarrage, sans liste pour les montrer.
  itemsOf(l.id).forEach((i) => remove("list_items", i.id));
  remove("grocery_lists", l.id);
  ep.draft = null;
  closeSheet();
  if (ep.listId === l.id) naviguer("arriere", () => { openList(null); rerender(); });
  else rerender();
}

// Nouvel ordre des listes, après un glisser-déposer (lib/glisser.js).
document.addEventListener("reorder", (e) => {
  if (e.target.id !== "ep-lists") return;
...
// ----------------------------------------------------------------- gestes ---
// Geste sur un article, du pointerdown au pointerup : glissement horizontal
// (poubelle) ou appui long (note et allée).
let gesture = null;
...
document.addEventListener("click", (e) => {
  ...
  switch (btn.dataset.ep) {
    // écran des listes
    case "open": return naviguer("avant", () => { openList(btn.dataset.id); rerender(); });
    case "new-list": return openListForm(null);
    case "edit-list": return openListForm(btn.dataset.id);

    // écran d'une liste
    case "back": return naviguer("arriere", () => { openList(null); rerender(); });
    ...
    case "checked-menu": return openCheckedMenu();
...
document.addEventListener("change", (e) => { if (e.target.id === "item-note") saveNote(e.target.value); });
// Quitter le champ de note (« terminé » / « OK » du clavier iOS, qui ne passe
// pas par la touche Entrée) enregistre et referme la feuille. Sauf si on vient
// de toucher autre chose dans la feuille : ce geste-là la refermera lui-même.
document.addEventListener("focusout", (e) => {
  if (e.target.id !== "item-note" || e.relatedTarget?.closest?.("#sheet")) return;
  saveNote(e.target.value);
  setTimeout(() => { if (document.getElementById("item-note") === e.target) closeSheet(); }, 120);
});
document.addEventListener("keydown", (e) => {
  if (e.key === "Enter" && e.target.id === "item-note") { e.preventDefault(); e.target.blur(); closeSheet(); }
});
