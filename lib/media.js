...
// Seul un vrai refus du serveur (4xx) abandonne un fichier. Une panne sans
// statut (Safari dit « Load failed », pas « Failed to fetch »), une session à
// rafraîchir ou une erreur 5xx : on garde le fichier pour le prochain essai.
function refusDefinitif(error) {
  const status = Number(error.status ?? error.statusCode);
  return navigator.onLine && status >= 400 && status < 500 && ![401, 408, 429].includes(status);
}
...
export function flushUploads() {
  flushing ||= (async () => {
    const cache = await openCache();
    // La file est relue à chaque tour : un fichier rangé pendant l'envoi d'un
    // autre (photos d'une même recette) part dans la même passe.
    const vus = new Set();
    for (let path; (path = readPending().find((p) => !vus.has(p))); ) {
      vus.add(path);
      ...
      if (hit) {
        ...
        if (error) {
          if (!refusDefinitif(error)) break;   // plus tard
