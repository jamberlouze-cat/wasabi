...
import {
  saveLocal, getUrl, hydrate, prefetch, signedUrl, compressImage,
...
function tileHtml(r) {
  const inner = `
    ...
    <span class="tile-name">${esc(r.title)}</span>`;
  ...
  const main = r.type === "url"
    ? `<a class="tile-main" href="${esc(r.url)}" target="_blank" rel="noopener noreferrer">${inner}</a>`
...
function refreshGrid() {
  ...
  document.getElementById("rc-filters").innerHTML = `
    <div class="puces">
      <button class="puce${rc.cat ? "" : " on"}" data-rc="cat" data-id="">${T.toutes}</button>
      ${cats.map((c) => `<button class="puce${rc.cat === c.id ? " on" : ""}" data-rc="cat" data-id="${c.id}">${esc(c.name)}</button>`).join("")}
    </div>
    ${store.data.tags.length ? `<button class="btn btn-small${rc.tags.size ? " on" : ""}" data-rc="tag-filter">${icon("etiquette")} ${rc.tags.size ? T.tagsActifs(rc.tags.size) : T.tags}</button>` : ""}`;
...
function openTagFilter() {
  openSheet(`
    <h2>${T.filtrerParTags}</h2>
    <p class="help">${T.filtrerParTagsAide}</p>
    <div class="pills">
      ${allTags().map((t) => `<button class="pill${rc.tags.has(t.id) ? " on" : ""}" data-rc="toggle-filter-tag" data-id="${t.id}">${esc(t.name)}</button>`).join("")}
    </div>
    <button class="btn btn-block" data-rc="close-sheet">${T.fermer}</button>`);
}
...
function detailHtml(r) {
  ...
  return `
    ...
    ${r.type === "url" ? `
      <a class="btn btn-primary btn-block" href="${esc(r.url)}" target="_blank" rel="noopener noreferrer">${icon("externe")} ${T.ouvrirRecette}</a>
  ...
  if (cover) {
    ...
    cover.innerHTML = `
      <div class="cover-edit">
        ...
        ${has ? `<button type="button" class="btn" data-rc="remove-cover">${T.retirer}</button>` : ""}
      </div>`;
    hydrate(cover);
  }
...
async function saveForm() {
  ...
  try {
    ...
    if (!coverSource && !cover_path && d.type === "file") coverSource = d.files.find((f) => f.blob && f.mime !== "application/pdf")?.blob || null;
    if (coverSource) {
    ...
    const before = new Set(d.isNew ? [] : tagIdsOf(d.id));
    for (const t of d.tagIds) if (!before.has(t)) put("recipe_tags", { id: idStable(d.id, t), recipe_id: d.id, tag_id: t, deleted_at: null });
    for (const t of before) if (!d.tagIds.has(t)) remove("recipe_tags", idStable(d.id, t));
...
// ------------------------------------------------------------ visionneuse ---
// Photos : pincer pour zoomer, double toucher, glisser entre les pages.
// PDF : aperçu intégré + lien vers la visionneuse du téléphone (iOS ne montre
// que la première page d'un PDF intégré).
const viewer = { files: [], index: 0, scale: 1, x: 0, y: 0, pointers: new Map(), start: null, lastTap: 0 };

function openViewer(recipeId, index = 0) {
  viewer.files = filesOf(recipeId);
  if (!viewer.files.length) { go("detail", recipeId); return; }
  viewer.index = Math.min(index, viewer.files.length - 1);
...
async function showPage() {
  const el = document.getElementById("viewer");
  const f = viewer.files[viewer.index];
  ...
  el.innerHTML = `
    <div class="viewer-bar">
      ...
      <span>${many ? `${viewer.index + 1} / ${viewer.files.length}` : ""}</span>
      <span class="viewer-nav">
        ...
        <button class="icon-btn" data-rc="next-page" ${viewer.index < viewer.files.length - 1 ? "" : "disabled"} aria-label="${T.pageSuivante}">${icon("suivant")}</button>` : ""}
      </span>
    </div>
    <div class="viewer-stage" id="viewer-stage"><div class="spinner"></div></div>`;
  const stage = document.getElementById("viewer-stage");
  const url = await getUrl(f.path);
  if (viewer.files[viewer.index] !== f || !stage.isConnected) return;
  if (!url) { stage.innerHTML = `<p class="viewer-msg">${T.fichierIndisponible}</p>`; return; }
  if (f.mime_type === "application/pdf") {
    stage.innerHTML = `<iframe class="viewer-pdf" src="${url}" title="PDF"></iframe>
      <a class="btn viewer-open" id="viewer-open" target="_blank" rel="noopener" hidden>${icon("externe")} ${T.ouvrirPdf}</a>`;
    const link = await signedUrl(f.path);
...
function turnPage(delta) {
  ...
  if (i < 0 || i >= viewer.files.length) return;
  viewer.index = i;
  showPage();
}
...
document.addEventListener("click", (e) => {
  ...
  switch (btn.dataset.rc) {
    ...
    case "cat": rc.cat = btn.dataset.id || null; return refreshGrid();
    case "tag-filter": return openTagFilter();
...
document.addEventListener("keydown", (e) => {
  ...
  if (e.key === "Escape" && !document.getElementById("viewer")?.hidden) closeViewer();
});
