...
// Une tuile ouvre toujours la fiche (lien, recette écrite, photos / PDF). Trois vues :
...
// Profondeur de chaque vue : aller plus loin glisse vers l'avant, revenir vers l'arrière.
const PROFONDEUR = { grid: 0, detail: 1, form: 2 };
function go(view, id = null) {
  const sens = PROFONDEUR[view] > PROFONDEUR[rc.view] ? "avant" : PROFONDEUR[view] < PROFONDEUR[rc.view] ? "arriere" : null;
  document.activeElement?.blur();
  const maj = () => {
    rc.view = view; rc.id = id;
    if (els.screen) { els.screen.innerHTML = ""; rerender(); els.screen.scrollTop = 0; }
  };
  if (sens && els.screen?.isConnected) naviguer(sens, maj); else maj();
}
...
function tileHtml(r) {
  ...
  return `
    <div class="tile${r.cover_path ? "" : " sans-image"}" data-media-box>
      <button class="tile-main" data-rc="detail" data-id="${r.id}">${inner}</button>
    </div>`;
}
...
document.addEventListener("keydown", (e) => {
  ...
  // « Rechercher » du clavier : la grille est déjà filtrée, on range le clavier.
  else if (e.key === "Enter" && e.target.id === "rc-q" && !e.isComposing) { e.preventDefault(); e.target.blur(); }
