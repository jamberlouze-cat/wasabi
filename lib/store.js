...
// Une requête qui n'a jamais atteint Supabase revient avec status 0. Les
// pannes passagères (session à rafraîchir, trop de requêtes, serveur en pause
// ou en erreur 5xx) comptent comme le réseau : on garde le geste et on réessaie,
// au lieu de le jeter comme un vrai refus.
const TRANSITOIRE = new Set([0, 401, 408, 429]);
const isNetworkFailure = (res) => !!res.error &&
  (TRANSITOIRE.has(res.status) || res.status >= 500 || !navigator.onLine);
