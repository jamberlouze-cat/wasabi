...
const CACHE_KEY = "wasabi-cache-v1";
const QUEUE_KEY = "wasabi-queue-v1";
...
// { type: 'upsert' | 'update', table, id, values }
...
export function saveCache() {
  ...
  writeJson(CACHE_KEY, {
    ...
    members: store.members, data: store.data, syncedAt: store.syncedAt,
  });
}
...
export function clearOfflineData() {
  ...
  store.members = []; store.data = emptyData(); store.syncedAt = null;
}
...
function enqueue(op) {
  const prev = queue.find((q) => q.table === op.table && q.id === op.id && !q.sending);
  if (prev) {
...
/** Modifie seulement les champs donnés. */
export function patch(table, id, fields) {
  act({ type: "update", table, id, values: { ...fields } });
}

/** Suppression logique. */
export function remove(table, id) {
  patch(table, id, { deleted_at: new Date().toISOString() });
}
...
function sendOp(op) {
  ...
  return supabase.from(op.table).update(op.values).eq("id", op.id);
}
