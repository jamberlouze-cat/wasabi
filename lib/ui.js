...
// ------------------------------------------------------------ navigation ---
// Passage d'un écran à l'autre, comme sur iOS : en entrant (« avant »), le
// nouvel écran arrive de la droite ; en revenant (« arriere »), l'écran quitté
// repart vers la droite. Le même geste partout : listes, recettes, réglages.
// `maj` change l'écran ; les animations sont dans app.css (§ navigation).
let navTour = 0;
export function naviguer(sens, maj) {
  const screen = document.getElementById("screen");
  if (!screen || matchMedia("(prefers-reduced-motion: reduce)").matches) { maj(); return; }
  const root = document.documentElement;
  const tour = ++navTour;
  if (document.startViewTransition) {
    root.dataset.nav = sens;
    const vt = document.startViewTransition(maj);
    vt.ready.catch(() => {});   // transition sautée (app en arrière-plan…) : l'écran change quand même
    vt.finished.catch(() => {}).finally(() => { if (navTour === tour) delete root.dataset.nav; });
    return;
  }
  // Navigateur sans View Transitions : le nouvel écran glisse seul.
  maj();
  screen.classList.remove("nav-avant", "nav-arriere");
  void screen.offsetWidth;   // relance l'animation
  screen.classList.add(`nav-${sens}`);
}
...
export function initViewport() {
  ...
  // Un champ qui reçoit le focus dans l'écran qui défile doit rester visible
  // au-dessus du clavier : iOS ne le fait pas pour nous dans cette mise en page.
  const montrer = () => {
    const el = document.activeElement;
    const screen = document.getElementById("screen");
    if (!el || !screen?.contains(el) || !el.matches("input, textarea, select")) return;
    const r = el.getBoundingClientRect(), s = screen.getBoundingClientRect();
    if (r.top >= s.top + 8 && r.bottom <= s.bottom - 8) return;
    screen.scrollTo({ top: screen.scrollTop + (r.top - s.top) - Math.max(16, (s.height - r.height) / 3), behavior: "smooth" });
  };
  document.addEventListener("focusin", () => { setTimeout(montrer, 80); setTimeout(montrer, 400); });
  if (vv) vv.addEventListener("resize", () => setTimeout(montrer, 50));
}
